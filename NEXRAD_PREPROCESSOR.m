%% Camp and Bear Fire KBBX Reflectivity data for tracking fire perimeters
...
prompt = 'Enter Fire Name: bear or camp:';
str = input(prompt,'s');
disp(str)
if isempty(str)
    str = 'camp';
...
if strcmp(cases,'camp')
    ...
    cd('~/Dropbox/CAMP_FIRE_RADAR/');
    files=dir('~/Dropbox/CAMP_FIRE_RADAR/KBBX_V06_2018*.nc');
    fout='camp_fire_radar_for_perimeters.mat'

elseif  strcmp(cases,'bear')
    
    cd('~/Dropbox/BEAR_FIRE/');
    files=dir('~/Dropbox/BEAR_FIRE/KBBX_V06_2020*.nc');
    fout='bear_fire_radar_for_perimeters.mat'
end
    
%% READ STATION METADATA FROM THE NETCDF FILE
StationLatitude = ncreadatt(files(1).name,'/','StationLatitude');
StationLongitude = ncreadatt(files(1).name,'/','StationLongitude');
StationElevationInMeters= ncreadatt(files(1).name,'/','StationElevationInMeters');
...
if pflag==1
    dbzx=squeeze(nanmax(dbZmaster(:,:,:),[],1));
