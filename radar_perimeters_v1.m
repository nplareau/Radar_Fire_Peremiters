...
if strcmp(cases,'camp')
    ...
    load('~/Dropbox/CAMP_FIRE_RADAR/camp_fire_radar_for_perimeters.mat','dbZmaster','timemaster','xmaster','ymaster','zmaster','StationElevationInMeters','StationLatitude','StationLongitude');
%     xmat=double(xdist); % I may have saved a single, so convert to double
%     ymat=double(ydist);
...
elseif strcmp(cases,'bear')
    ...
    load('~/Dropbox/BEAR_FIRE/bear_fire_radar_for_perimeters','dbZmaster','timemaster','xmaster','ymaster','zmaster','StationElevationInMeters','StationLatitude','StationLongitude');
%     xmat=double(xdist); % I may have saved as a single, so convert to double
%     ymat=double(ydist); %convert to double
...
pflag=1; %plotting flag (1= yes, 0= no)
...
for tt=startf:2:size(dbZmaster,1)-1% for each time step of the radar data (moves in strides of 2 because we take the maximum for each r,azimuth point between the two PRF sweeps
    ...
    zmax1(zmax1<5)=nan; %remove non-smoke related returns (dbz threshold of 5 here is somewhat arbitrary)
    zmax1(isnan(zmax1))=0; %nan points = bad data too
    zmax1=medfilt2(zmax1,[3 3]); % 3x3 median filter of the radar data to favor smooth results
    xmat=squeeze(xmaster(tt,:,:));
    ymat=squeeze(ymaster(tt,:,:));
...
%                     y2(didx+200:end)=[];
    ...
    %     export_fig(sprintf('%s%s.png',ofig,datestr(timemaster(tt),'yyyymmddHHMM')));
        
end

%%
% im2gif('camp_perimeter_lobes.tif','-delay',.2);
%      imwrite(im,map,'camp_fire_perimeters.gif','DelayTime',0.2,'LoopCount',inf);
